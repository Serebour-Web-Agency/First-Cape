// SmartHub Buy Properties Loader
...
class PropertiesLoader {
  ...
  async init() {
    console.log('[Properties] Initializing v3.3 (CSP Compliant)...');
  ...
  async loadProperties() {
    try {
      ...
      const filterFormula = `AND(
        {${config.propertyFields.listingType}} = "Buy",
      ...
      this.properties = records
        ...
        .filter(property => {
          ...
          if (!hasMedia) {
            console.warn('[Properties] Property missing media:', property.name);
          }
          return hasMedia;
        });
      ...
      console.log(`[Properties] Loaded ${this.properties.length} buy properties with valid media`);
  ...
  transformRecord(record) {
    ...
    if (fields[fieldMap.cdnGalleryJSON]) {
      try {
        ...
        console.warn('[Properties] Failed to parse CDN Gallery JSON for', record.id, e);
      }
    }
  ...
  openNotifyModal(city) {
    console.log('[Properties] Opening notify modal for:', city);
    ...
    p.innerHTML = `We'll notify you when new properties become available in <strong>${city}</strong>`;
  ...
  async submitNotifyForm(city, form) {
    ...
    const notes = `Property Notification Request for ${city} - User wants to be notified when properties become available in this city.`;
    ...
    try {
      ...
      console.log('[Properties] Notify form submitted successfully');
      ...
      p1.innerHTML = `We'll notify you when properties become available in <strong>${city}</strong>.`;
      ...
      console.error('[Properties] Notify form error:', error);
  ...
  renderProperties() {
    ...
    if (!container) {
      console.error('[Properties] Container element #propertiesContainer not found');
      return;
    }
  ...
  createPropertyCard(property) {
    ...
    // Add data attributes for favorites system
    card.setAttribute('data-property-id', property.id);
    card.setAttribute('data-bedrooms', property.bedrooms || '');
    card.setAttribute('data-bathrooms', property.bathrooms || '');
    card.setAttribute('data-property-type', property.propertyType || '');
    ...
    card.innerHTML = `
      ...
      <div class="property-card__body">
        <h3 class="property-card__title property-name">${property.name}</h3>
        ${location ? `<div class="property-card__meta property-city">
          <span>${location}</span>
          ${property.bedrooms ? `<span>${property.bedrooms} bed</span>` : ''}
          ${property.bathrooms ? `<span>${property.bathrooms} bath</span>` : ''}
        </div>` : ''}
        <div class="property-card__price-row">
          <div class="property-card__price-main property-price">${formattedPrice}</div>
...
document.addEventListener('DOMContentLoaded', () => {
  console.log('[Properties] DOM ready, initializing v3.3 (CSP Compliant)...');
